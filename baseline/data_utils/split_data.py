...
class DataSplitter:
    def __init__(
        ...
        self.input_events: Dict[str, pl.DataFrame] = {}
        self.target_events: Dict[str, pl.DataFrame] = {}
    ...
    def _create_input_chunk(
        ...
        Args:
            event_df (pl.DataFrame): A DataFrame storing all events.
            train_target_start (datetime):  The first date in train target time-window.
        Returns:
            pl.DataFrame: the function returns DataFrame with input data.
        """
        train_input = event_df.filter(pl.col("timestamp") < train_target_start)
        return train_input
    ...
    def _create_target_chunks(
        ...
        Args:
            event_df (pl.DataFrame): A DataFrame storing all events.
            properties_df (pl.DataFrame): Product properties.
            train_target_start (datetime): The first date in train target time-window.
            validation_target_start (datetime): The first date in validation target time-window.
        Returns:
            tuple[pl.DataFrame, pl.DataFrame]: Two DataFrames: first storing train target events,
            second storing validation target events.
        """
        train_target = event_df.filter(
            (pl.col("timestamp") >= train_target_start)
            & (pl.col("timestamp") < validation_target_start)
        )
        validation_target = event_df.filter(
            (pl.col("timestamp") >= validation_target_start)
            & (pl.col("timestamp") <= self.end_date)
        )
    ...
    def split(self) -> None:
        ...
        for event_type in EventTypes:
            ...
            events = events.with_columns(pl.col("timestamp").str.to_datetime(time_unit="ms"))
            ...
            if event_type == "product_buy":
                properties = pl.read_parquet(self.challenge_data_dir.properties_file)
    ...
    def save_splits(self) -> None:
        ...
        for event_type, events in self.input_events.items():
            ...
            events.write_parquet(
                self.challenge_data_dir.input_dir / f"{event_type}.parquet"
            )
    ...
    def load_events(self, event_type: EventTypes) -> pl.DataFrame:
...
def main():
    ...
    product_buy = pl.read_parquet(
        challenge_data_dir.data_dir / f"{EventTypes.PRODUCT_BUY.value}.parquet",
    )
    end_date = product_buy.get_column("timestamp").str.to_datetime(time_unit="ms").max()
