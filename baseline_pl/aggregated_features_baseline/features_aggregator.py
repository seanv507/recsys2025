...
from baseline_pl.aggregated_features_baseline.calculators import (
    Calculator,
...
def get_top_values(
    events: pl.DataFrame, columns: List[str], top_n: int
) -> dict[str, pl.Series]:
    """
    Extracts the top_n most frequent values for specified columns in event data.

    Args:
        events (pl.DataFrame): DataFrame containing event data.
    ...
    """
    logger.info("calculate top_values")
    top_values = dict(
        events
        .select(
            pl.col(columns)
...
class FeaturesAggregator:
    ...
    def _filter_events_to_relevant_clients(
        ...
        """
        return df.filter(pl.col("client_id").is_in(self._relevant_client_ids))
    ...
    def generate_features(
        ...
        Args:
            ...
            df (pl.DataFrame): DataFrame containing the event data.
        ...
        for client_id, events in tqdm(df.group_by(client_id_column)):
